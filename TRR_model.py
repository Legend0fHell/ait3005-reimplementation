...
chain_reasoning = reasoning_template | model2
...
def attention_phase(G, current_time, lambda_decay, q=6):
    """
    Uses PageRank to find important entities and their connections
    ...
    for u, v, data in G_filtered.edges(data=True):
        ...
        try:
            ...
            # Mark edges with future timestamps for removal
    ...
    print(f"Filtered out {len(edges_to_remove)} future edges from graph for PageRank calculation")
    
    # Apply weight decay to remaining edges
    G_filtered = update_edge_decay_weights(G_filtered, current_time=current_time, lambda_decay=lambda_decay)

    # # Personalize vector if it is the required stocks
    # personalize_vector = dict()
    # for node in G_filtered.nodes():
    #     if G_filtered.nodes[node].get("type") == "stock":
    #         personalize_vector[node] = 0.01
    #     else:
    #         personalize_vector[node] = 1/len(G_filtered.nodes())
            
    # Compute PageRank scores
    pr_scores = nx.pagerank(G_filtered, weight="weight")
    ...
    # Create the subgraph from the filtered graph
    ...
    # Print top 10 nodes by number of incoming edges
...
# Final Reasoning
# ---------------------------
def final_reasoning(G, portfolio, portfolio_sector):
    """
    Convert the graph into a tuple string and run the final reasoning chain.
    """
    tuples_str = graph_to_tuples(G)
    
    # Save the tuples to a file for debugging
    with open("tuples.txt", "w", encoding="utf-8") as f:
        f.write(tuples_str)
    
    # Create portfolio string once
    portfolio_str_full = ", ".join([f"{stock}-{sector}" for stock, sector in zip(portfolio, portfolio_sector)])

    print("\nInput tuples for reasoning:")
    ...
    # Use retry logic for final reasoning
...
def trr(df: pd.DataFrame, prediction_date: str, load_saved_graph: bool = False, lambda_decay: float = 1, 
    ...
    """
    # Convert prediction date to timestamp
    try:
        pred_ts = isoparse(str(prediction_date)).timestamp()
        print(f"Prediction timestamp: {pred_ts}")
    except Exception as e:
    ...
    if load_saved_graph:
        ...
        try:
            with open("knowledge_graph_p3.pkl", "rb") as f:
                G = pickle.load(f)
            print(f"Loaded graph with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
        except Exception as e:
    ...
    print(f"Using prediction date: {prediction_date}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Temporal Relational Reasoning Model")
    parser.add_argument("--news_from", type=int, default=1, help="Starting index for news")
    parser.add_argument("--news_to", type=int, default=9400, help="Ending index for news")
    parser.add_argument("--pred_date", type=str, default="2025-04-02T01:00:00+07:00", help="Prediction date")
    parser.add_argument("--summarize", action="store_true", help="Generate summarized news")
    parser.add_argument("--load_graph", action="store_true", help="Load existing knowledge graph")
    parser.add_argument("--lambda_decay", type=float, default=1.0, help="Lambda decay parameter")
    parser.add_argument("--q", type=int, default=6, help="Top-q entities to select")
    parser.add_argument("--max_frontier_size", type=int, default=10, help="Maximum number of entities to process in a single batch")
    parser.add_argument("--batch_size", type=int, default=5, help="Batch size for news summarization")
    parser.add_argument("--no_threading", action="store_true", help="Disable multithreading")
    parser.add_argument("--max_workers", type=int, default=5, help="Maximum number of worker threads")
    parser.add_argument("--skip", type=int, default=0, help="Number of articles to skip in processing")
    parser.add_argument("--graph_checkpoint", type=str, help="Path to knowledge graph checkpoint file to load")
    parser.add_argument("--canonical_checkpoint", type=str, help="Path to canonical entities checkpoint file to load")
    parser.add_argument("--eval_mode", action="store_true", help="Run in evaluation mode")
    parser.add_argument("--eval_start", type=str, help="Start date for evaluation (format: same as pred_date)")
    parser.add_argument("--eval_end", type=str, help="End date for evaluation (format: same as pred_date)")
    ...
    if args.eval_mode:
        if not (args.eval_start and args.eval_end):
            print("Error: eval_start and eval_end are required for evaluation mode")
            return
        
        print(f"Running in evaluation mode from {args.eval_start} to {args.eval_end}")
        return evaluate_date_range(
            args.eval_start,
            args.eval_end,
    ...
    # Regular mode (single prediction)
    # Read the news data
    print(f"Reading news data from index {args.news_from} to {args.news_to}...")
    df = read_news_data("cleaned_posts.csv")
    df = df.iloc[args.news_from:args.news_to+1]
    ...
    # Generate summarized news if requested
    if args.summarize:
        print("Generating summarized news...")
        df_summarized = make_summarized_news(df, batch_size=args.batch_size)
    else:
        print("Loading existing summarized news...")
        df_summarized = read_news_data("summarized_articles.csv")
    
    # Run TRR model
    print(f"Running TRR for prediction date: {args.pred_date}")
    prediction = trr(
        df_summarized, 
        args.pred_date,
